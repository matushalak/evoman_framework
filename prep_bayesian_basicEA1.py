...
def basic_ea (hyperparameters:tuple, n_hidden_neurons:int,
              experiment_name:str, env:Environment, save_gens:bool = True, new_evolution:bool = True):
    ''' 
    Basic evolutionary algorithm to optimize the weights 
    '''

    scaling_factor, sigma_prime, alpha, tournament_size, elite_fraction, mr, cr, popsize, max_gen = hyperparameters   
    ...
    if new_evolution == True:
        ...
        #elite_fraction = 0.8
        starting_elite_fraction = elite_fraction
    ...
    for i in range(max_gen):
        ...
        shared_fitnesses = vectorized_fitness_sharing(fitnesses, population, gene_limits, k=scaling_factor)
        # shared_fitnesses = fitnesses # disables fitness sharing
        # ?? crowding ??
        # ?? speciation - islands ??

        # Parent selection: (Tournament? - just first try) Probability based - YES
        parents, parent_fitnesses = vectorized_parent_selection(population, shared_fitnesses, env, k=tournament_size)
